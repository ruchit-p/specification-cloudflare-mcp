 ...
 * @param userPermissions Array of user permissions from the token
...
/**
 * Helper to create a permission-restricted tool handler
 * @param permission Required permission
 * @param handler The actual tool handler
 * @returns Wrapped handler that checks permissions first
 */
...
export async function authorize(c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) {
  ...
  if (!mcpClientAuthRequest.clientId) {
    console.error('[Auth0 Authorize] No clientId in request');
    return c.text("Invalid request", 400);
  }

  const client = await c.env.OAUTH_PROVIDER.lookupClient(mcpClientAuthRequest.clientId);
  console.log('[Auth0 Authorize] Looked up client:', JSON.stringify(client));
  
  if (!client) {
    console.error('[Auth0 Authorize] Client not found:', mcpClientAuthRequest.clientId);
  ...
  console.log('[Auth0 Authorize] Set cookie with secure:', isSecure);
...
export async function confirmConsent(
  ...
  console.log('[Auth0 Consent] Request URL:', c.req.url);
  ...
  console.log('[Auth0 Consent] Form data:', { transactionState, consentToken, consentAction });
  ...
  // Add resource indicator for 2025 MCP spec compliance (RFC 8707)
  authorizationUrl.searchParams.set("resource", c.env.AUTH0_AUDIENCE);
  
  console.log('[Auth0 Consent] Redirecting to Auth0:', authorizationUrl.href);
  console.log('[Auth0 Consent] Callback URI:', new URL("/callback", c.req.url).href);
...
export async function callback(c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) {
  console.log('[Auth0 Callback] Request URL:', c.req.url);
  console.log('[Auth0 Callback] Query params:', Object.fromEntries(new URL(c.req.url).searchParams.entries()));
  
  // Check for OAuth errors first
  const error = c.req.query("error");
  if (error) {
    const errorDescription = c.req.query("error_description") || "An error occurred during authentication";
    console.error(`[Auth0 Callback] OAuth error: ${error} - ${errorDescription}`);
    
    // Handle specific Auth0 error cases
    if (error === "login_required") {
      return c.text("Login required. Please authenticate again.", 401);
    } else if (error === "consent_required") {
      return c.text("Consent required. Please approve the requested permissions.", 403);
    } else if (error === "access_denied") {
      return c.text("Access denied. You declined the authorization request.", 403);
    }
    
    return c.text(`Authentication failed: ${errorDescription}`, 400);
  }
...
/**
 * Clear authentication state for a user (logout)
 * This function should be called when a user explicitly logs out
 */
export async function clearAuthenticationState(
  ...
  try {
    // Clear any stored tokens or session data for this user
    // This is a best-effort attempt to clear authentication state
    
    // If we have access to the OAuth provider's internal storage,
    // we would clear the tokens here
    
    // For now, we'll log the logout action
    console.log(`User ${userId} logged out at ${new Date().toISOString()}`);
    
    return { success: true, message: "Authentication state cleared" };
  } catch (error) {
                ...
                <div class="container">
                    <div class="card">
                        <div class="header">
                            ...
                            <h1>Specification MCP Server - Authorization Request</h1>
                            <a href="${clientUri}" target="_blank" rel="noopener noreferrer" class="app-link">${clientName}</a>
                        </div>

                        <p class="description">
                            <strong>${clientName}</strong> is requesting permission to access the <strong>Specification API</strong> using your
